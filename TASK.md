...
## 📌 Latest — Art integration + App Store metadata (2026-08-06, branch `claude/pre-launch-audit-checklist-7dwc53`)

- [x] **The 26 generated assets wired in.** `scripts/build_image_assets.py`
      derives the catalog from the masters in `assets/` — @2x/@3x only (iOS 26
      deployment target means no @1x device exists). Constellations are
      trimmed and re-centred, because the real star patterns run from Cancer's
      30% of frame to Virgo's 75% and would otherwise render at wildly
      different sizes in a 36pt avatar. Planets keep full RGBA; palette
      quantisation bands the Sun's corona.
- [x] **Every asset on a real screen** — placement hero, forecast and
      transparency rows, the retrograde strip, People avatars keyed to Sun
      sign, both empty states, the daily reveal, and the SceneKit moon.
      `ImageAssetTests` fails the build if any name stops resolving.
- [x] **App Store metadata pack.** `metadata/app-store.json` is the source of
      truth; `docs/aso/` holds the reasoning, screenshot storyboard and
      privacy answers. Three keyword fields (en-US + es-MX + en-GB
      cross-localization) take indexed space from 127 characters to 347.
- [x] **`scripts/aso_lint.py`** — field limits, cross-field token overlap
      (stems included), unshipped-feature claims, hosted URLs with no page,
      and support addresses at unregistered domains. In CI and
      `local_checks.sh`. `--fastlane` writes the tree `deliver` uploads.
- [x] **App Store screenshots generated by CI** at exactly 1320 × 2868, with a
      test asserting the dimensions. Step 8 no longer needs a Mac; nothing in
      the runbook does.
- [x] **Five more accuracy defects fixed** — Help claiming palm analysis, four
      palmistry glossary terms, `PalmHubView` claiming a model that doesn't
      exist, the People screenshot's stale privacy line, and `tarot` in the
      keywords. See the 2026-08-06 status block in `LAUNCH-READINESS.md`.
- [x] **Dead support addresses replaced.** Everything published pointed at
      `lumina.app`, never registered, so all mail bounced.
- [!] **Not done, and not doable from here:** revoke the leaked Anthropic key,
      deploy the backend to Fly, enable GitHub Pages, configure RevenueCat,
      submit. All account-gated — `LAUNCH-STEPS.md` is the runbook.

---

## Pre-launch audit + remediation (2026-08-04, branch `claude/pre-launch-audit-checklist-7dwc53`)
