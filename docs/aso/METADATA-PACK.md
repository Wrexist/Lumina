...
- [ ] Description, promo text, What's New pasted
      *(or run `python3 scripts/aso_lint.py --fastlane` and `fastlane deliver`,
      which uploads all of the above plus the screenshots in one command)*
- [ ] Screenshots uploaded — five are generated by CI at exactly 1320 × 2868;
      download the `app-store-screenshots` artifact from the latest green run
