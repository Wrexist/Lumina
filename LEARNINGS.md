...
## [2026-07-01] App capabilities pass — IAP/push/auth/universal-links/3D Moon

Full plan at `docs/CAPABILITIES-PLAN.md`. Built via 5 parallel subagents on
disjoint files (one shared-file conflict — `SettingsView.swift` — resolved by
having the IAP agent own the subscription rows and doing the Sign-in row
myself afterward). All lessons below are from that pass.

**[2026-07] SwiftLint's opt-in `object_literal` rule flags any `UIColor(...)`/`UIImage(...)` init whose arguments are ALL literals**
First real CI failure from this pass: `MoonSphere3DView.swift`'s procedural SceneKit texture/lighting used `UIColor(white: 0.05, alpha: 1)`-style calls with literal constants, and `object_literal` (enabled in `.swiftlint.yml`'s `opt_in_rules`) flagged 6 of them — it wants `#colorLiteral(...)`, which makes no sense for a runtime-generated texture. The rule inspects the syntax at the actual initializer call site: an argument that's a variable/parameter reference doesn't trip it, even if that variable's value came from a literal elsewhere. Fix: route the call through a tiny `private static func gray(_ white: Double, alpha: Double = 1) -> UIColor { UIColor(white: white, alpha: alpha) }` helper — inside the helper the arguments are parameter references, not literals, so the call site is clean. This is a real fix, not a suppression comment, and reads better besides. Calls that already used a variable (e.g. `UIColor(white: shade, alpha: 0.85)` where `shade` is computed per-crater) never tripped the rule and needed no change — first-time procedural-color-generation code in this repo, so no prior precedent existed for this trap.

**[2026-07] `SCNView.allowsCameraControl = true` gives free drag-orbit/pinch-zoom — don't hand-roll gesture code for a "look around" 3D view**
For a from-scratch immersive 3D view (a phase-accurate Moon sphere) with no prior 3D-rendering code in this repo, SceneKit (not RealityKit) was the deliberate choice: RealityKit's `RealityView` API is newer and riskier to get exactly right without a local compiler; SceneKit's `SCNView`/`SCNScene`/`SCNNode`/`SCNLight` surface has been stable since iOS 8 and is far higher-confidence to write blind. `view.allowsCameraControl = true` is the single line that replaces all custom `DragGesture`/`MagnificationGesture` handling for orbit/zoom — use it before reaching for manual gesture code on any "let the user look around a 3D object" feature.

**[2026-07] Directional-light terminator math: position the light where the SUN actually is, not where intuition says**
`MoonPhaseResult.angle` is the real Sun-Moon-Earth phase angle (0°=new, 180°=full) already computed by the backend — genuine astronomical data, not decoration. With the camera on `+Z` looking at the origin, the naive `z = cos(angleRad) * radius` light-position formula is backwards: it fully lights the visible face at `angle=0` (new moon) and darkens it at `angle=180` (full moon) — exactly inverted. The fix is `z = -cos(angleRad) * radius`. General lesson: when a light's position is derived from a real physical angle via `look(at:)`, verify the mapping by working through both endpoints by hand (or a throwaway Python trig check) before trusting the "obviously right" formula — a backwards terminator is a silent, only-visually-detectable bug, and this app's whole brand promise is never faking astronomy.

**[2026-07] `nonisolated` delegate → `@MainActor` continuation bridge is the house pattern for ANY Apple delegate-based async API under Swift 6**
`BirthPlaceSearch`'s `MKLocalSearchCompleter` bridging (existing code) was reused twice more this pass: `AuthManager`'s `ASAuthorizationControllerDelegate` and `IAPManager`'s `PurchasesDelegate`. Shape: extract only `Sendable` fields from the delegate callback's (non-`Sendable`) argument into a small `Sendable` struct *inside* the `nonisolated` method, then `Task { @MainActor in }` (or resolve a `CheckedContinuation`) to hop back. Don't touch actor/MainActor state directly from the `nonisolated` callback. This is now a 3-for-3 pattern in this codebase — treat it as the default answer whenever wrapping a delegate-based Apple framework API (Vision, AVFoundation, StoreKit-adjacent SDKs, etc.) under strict concurrency.

**[2026-07] Wrapping third-party SDKs blind (no local Mac, no vendored source to read) is inherently a "verify via CI, iterate" task — budget for it**
`IAPManager` (RevenueCat), `PushNotificationManager` (OneSignal), and `SupabaseAuthService` (supabase-swift) were all written against each SDK's documented/recalled public API shape with zero ability to compile-check locally. Each subagent flagged its own confidence level per call site in its final report. This is the correct process (flag uncertainty explicitly rather than silently guess), and the first real CI failure of this pass was unrelated to any of the three (it was the `object_literal` lint issue above) — but expect the *next* failure, if any, to be a small rename/reshape inside one of these three files, not a structural problem.
...
**[2026-07-01] `com.apple.developer.usernotifications.channel` was not a real entitlement — a signed `ios-testflight.yml` archive is the only thing that actually validates entitlement keys**
Confirms the "verify via CI, iterate" prediction above, but from the entitlements side, not the SDK-wrapping side. `ci.yml`'s simulator build runs with `CODE_SIGNING_ALLOWED=NO`, so it never checks entitlement keys against a provisioning profile — it happily built for weeks with a made-up key sitting in `Lumina.entitlements`. Only the manual, signed `ios-testflight.yml` archive step actually asks Apple's tooling "is this a real entitlement for this profile," and it failed with `Entitlement com.apple.developer.usernotifications.channel not found and could not be included in profile` — the exact outcome the original doc comment (`project.yml`) and `CAPABILITIES-PLAN.md` §2 flagged as a risk when the key was added best-effort, unable to be verified against live Apple docs at the time. Fix was simply deleting the key — there is no Xcode-capability-editor equivalent for an APNs "broadcast/channel" entitlement today, so OneSignal's segment/broadcast sends rely on ordinary per-device APNs delivery (`aps-environment`), fanned out server-side, not on any app-level entitlement. General lesson: a best-effort/unverified entitlement key can sit invisible in an unsigned CI pipeline indefinitely — the first signed archive is the actual validation gate, not green CI.
