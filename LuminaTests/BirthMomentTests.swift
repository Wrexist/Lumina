...
final class BirthMomentTests: XCTestCase {
    ...
    func testCombineBuildsBirthInstantInBirthZone() {
        ...
        let pickedDay = pickerDate("Asia/Tokyo", year: 1994, month: 3, day: 2)
        let pickedTime = pickerDate("Asia/Tokyo", year: 2026, month: 7, day: 2, hour: 8, minute: 15)
        ...
        let expected = calendar("America/New_York").date(
        ...
        let newYorkParts = calendar("America/New_York")
    ...
    func testCombineWithoutTimeAnchorsNoon() {
        let tokyo = calendar("Asia/Tokyo")
        let pickedDay = pickerDate("Asia/Tokyo", year: 1995, month: 6, day: 15)
        ...
        let parts = calendar("America/New_York")
            .dateComponents([.year, .month, .day, .hour], from: birthDate)
        XCTAssertEqual(parts.day, 15)
        XCTAssertEqual(parts.hour, 12)
    }

    func testPickerValuesRoundTrip() {
        let tokyo = calendar("Asia/Tokyo")
        let pickedDay = pickerDate("Asia/Tokyo", year: 1988, month: 12, day: 31)
        let pickedTime = pickerDate("Asia/Tokyo", year: 2026, month: 7, day: 2, hour: 23, minute: 45)
        ...
        let timeParts = tokyo.dateComponents([.hour, .minute], from: pickers.time!)
    ...
    func testSharedBirthDataEncodesCalendarComponents() throws {
        let birthData = BirthData.fromPickers(
            pickedDay: pickerDate("Asia/Tokyo", year: 1996, month: 5, day: 21),
            pickedTime: nil,
            placeName: "Tokyo, Japan",
            latitude: 35.68,
            longitude: 139.69,
            timeZoneIdentifier: "Asia/Tokyo",
            deviceCalendar: calendar("Asia/Tokyo")
        )
        ...
        let day = calendar("Asia/Tokyo").dateComponents([.month, .day], from: decoded.birthDate)
